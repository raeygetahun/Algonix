...
const Motto = () => {
  ...
  const titleOne="Empowering Your Business"
  const titleTwo=""
  return (
...
<p className="text-[6vw]   text-[#11112B] ">
  ...
  
    {
      titleOne.split('').map((letter:string,index)=><motion.span
        className={`${index>10? " font-Poppins italic font-[300]":" font-Space font-bold "} `}
        initial="hidden"
        whileInView="visible"
  ...
  {/* ... repeat for each letter ... */}
        ...
        <motion.p initial="hidden" whileInView="visible" variants={pargVariants}  className="font-Space text-[6vw] text-[#11112B] font-bold ">
          <span>with </span>
           <span className={`change text-[#D5D9E5] text-[6vw]   `}>Cutting-Edge</span>
        ...
        <div   className="flex flex-row items-center space-x-4 mx-[3vw]  p-[1vw] mt-20">
          ...
          <Counter id="countClients" labelText="Satisfied Clients" num={10} channge={3} />
          <div className="w-[20vw] h-px bg-white"></div>
          <Counter id="countCountries" labelText="Handled Countries" num={4} channge={2} />
        </div>
      </div>
    </section>
  );
